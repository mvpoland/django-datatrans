...
USING_POSTGRESQL = DATABASE_ENGINE.startswith(
    "postgresql"
) or DATABASE_ENGINE.startswith("django.db.backends.postgresql")
        ...
        def test_concurrent_inserts_with_same_value_break_pre_save(self):
            ...
            def add_new_records():
                ...
                count_kv = KeyValue.objects.filter(
                    language=get_default_language(), value=value
                ).count()
                self.assertEqual(
                    count_kv,
                    1,
                    u"Got {} KeyValues after concurrent insert instead of 1.".format(
                        count_kv
                    ),
                )
